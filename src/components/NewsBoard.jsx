...
const NewsBoard = ({ category }) => {
  ...
  useEffect(() => {
    ...
    if (!apiKey) {
      console.error('API Key is undefined. Please check your environment variables.');
      return;
    }
  ...
  return (
    <div>
      <form onSubmit={handleSearch} className="flex justify-center mt-10 shadow-lg mr-12 ml-12 animate-bounce">
        ...
        />
        <button type="submit" className="px-3 py-2 bg-blue-700 hover:bg-blue-800 text-sm font-medium text-white rounded">
          <span className="flex items-center whitespace-nowrap px-3 py-[0.25rem] text-surface dark:border-neutral-400 dark:text-white [&>svg]:h-5 [&>svg]:w-5" id="button-addon2">
